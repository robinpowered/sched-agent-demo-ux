...
- **CustomTooltip.tsx** - Shared tooltip component with consistent dark styling
...
h1 { font-size: var(--text-2xl); font-weight: var(--font-weight-normal); line-height: 1.5; }
h2 { font-size: var(--text-xl); font-weight: var(--font-weight-normal); line-height: 1.5; }
h3 { font-size: var(--text-lg); font-weight: var(--font-weight-normal); line-height: 1.5; }
h4 { font-size: var(--text-base); font-weight: var(--font-weight-normal); line-height: 1.5; }
p  { font-size: var(--text-base); font-weight: var(--font-weight-normal); line-height: 1.5; }
label { font-size: var(--text-base); font-weight: var(--font-weight-normal); line-height: 1.5; }
button { font-size: var(--text-base); font-weight: var(--font-weight-normal); line-height: 1.5; }
...
--font-weight-normal: 400  /* Default for all elements */
--font-weight-bold: 700    /* For emphasized/bolded text */
```
...
```css
/* Light mode (default) */
--primary: #db2777          /* Pink primary color (legacy - being phased out) */
...
**Default Highlight Color**: `#2774C1` (blue) - This is the primary interactive color used throughout the application for:
- Meeting blocks in the calendar
...
**Note**: The pink `--primary` color is legacy and being phased out in favor of the blue `#2774C1` for consistency with the meeting block color scheme.
...
1. Add view to `View` type in `/types/index.ts`
...
1. Add filter field to `MeetingRoomFilters` interface in `/types/index.ts`
2. Update `getFilteredRooms()` logic in `/utils/index.ts`
...
3. Update room type detection in `getRoomType()` in `/utils/index.ts` if needed
4. Add to FiltersSidebar amenities list
5. Update AI room suggestion logic if relevant
6. Add Font Awesome icon mapping if needed
