...
interface PageLayoutProps {
  ...
  pageTabs?: { value: string; label: string }[];
  ...
  onSidebarStateChange?: (type: SidebarType, previousType?: string) => void;
  ...
  onWorkplaceExpandedChange?: (expanded: boolean) => void;
...
export function PageLayout({ 
  ...
  const [currentSidebarType, setCurrentSidebarType] = useState<SidebarType>(sidebarType || 'none');
  const [previousSidebarType, setPreviousSidebarType] = useState<string | undefined>();
  const [sidebarStack, setSidebarStack] = useState<Exclude<SidebarType, 'none'>[]>([]);
  ...
  const handleOpenSidebar = (newSidebarType: 'ai-assistant' | 'meeting-details' | 'create-meeting' | 'filters') => {
    ...
    onSidebarStateChange?.(newSidebarType, currentSidebarType);
  };
  ...
  const showBackButton = (currentSidebarType === 'ai-assistant' || currentSidebarType === 'meeting-details' || currentSidebarType === 'create-meeting' || currentSidebarType === 'filters') && (
    ...
    (hasDefaultSidebar && sidebarStack.length === 1 && currentSidebarType !== getDefaultSidebarType()) // On default sidebar page but not showing default
  );
  ...
  const SettingsFlyout = () => {
    ...
    return (
      ...
      >
        ...
        <div
          className={`px-4 py-2 hover:bg-gray-700 cursor-pointer text-sm whitespace-nowrap ${
            'settings-priority-support' === currentView ? 'text-white' : 'text-gray-300'
          }`}
    ...
    <div className="h-screen bg-gray-50 flex overflow-hidden max-w-full">
      ...
      <div className="flex-1 flex flex-col h-screen min-w-0 overflow-hidden">
        ...
        <header className={`bg-white border-b border-gray-200 ${
          meetingSpacesViewMode === 'month' && selectedMonthViewRoom ? '' : 'shadow-sm'
        } flex-shrink-0 max-[600px]:mr-0 relative z-10 ${
          currentSidebarType === 'ai-assistant' || currentSidebarType === 'meeting-details' || currentSidebarType === 'create-meeting' || currentSidebarType === 'room-details' ? 'mr-96' : 
        ...
        {currentView === 'map' ? (
          ...
          <div className={`flex-1 flex min-w-0 ${fullViewport ? "overflow-hidden" : "overflow-y-auto"} max-[600px]:mr-0 ${
            currentSidebarType === 'ai-assistant' || currentSidebarType === 'meeting-details' || currentSidebarType === 'create-meeting' || currentSidebarType === 'filters' || currentSidebarType === 'room-details' || currentSidebarType === 'service-ticket' ? 'mr-96' : ''
          }`}>
            {/* Map Content Area */}
          ...
          <div className={`flex-1 min-w-0 ${fullViewport ? "overflow-hidden" : "overflow-y-auto"} max-[600px]:mr-0 ${
        ...
        <div className="fixed top-0 right-0 h-full w-96 bg-white border-l border-gray-200 shadow-lg z-50 flex flex-col">
          ...
          {currentSidebarType === 'meeting-details' && (
            <MeetingDetailsSidebar
              selectedMeeting={selectedMeetingDetails}
