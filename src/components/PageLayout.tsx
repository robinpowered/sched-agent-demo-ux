...
// Import shared types
import { Message, ChatSession, Meeting, Room, SidebarType, View } from '../types';
...
export function PageLayout({
  ...
  const onWorkplaceExpandedChange = (expanded: boolean) => {
    ...
    if ((expanded && !isWorkplaceExpanded) || (!expanded && isWorkplaceExpanded)) {
      toggleWorkplaceExpanded();
    }
  };
  ...
  const generateDateOptions = () => {
    ...
    if (meetingSpacesViewMode === 'week' && currentView === 'meeting-spaces') {
      for (let i = -4; i < 13; i++) { // 4 weeks back, 12 weeks forward
    ...
    if (meetingSpacesViewMode === 'month' && currentView === 'meeting-spaces') {
      for (let i = -6; i < 13; i++) { // 6 months back, 12 months forward
  ...
  useEffect(() => {
    if (currentView === 'meeting-spaces' && meetingSpacesViewMode) {
      ...
      if (meetingSpacesViewMode === 'week') {
        ...
        setSelectedDate(`${sunday.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} - ${saturday.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`);
  ...
  React.useEffect(() => {
    ...
    if (sidebarType === 'meeting-details') {
      ...
      if (newStack.length === 0 || newStack[newStack.length - 1] !== 'meeting-details') {
    ...
    if (sidebarType === 'create-meeting') {
      ...
      if (newStack.length === 0 || newStack[newStack.length - 1] !== 'create-meeting') {
    ...
    if (hasDefaultSidebar && sidebarType !== 'none') {
      ...
      if (defaultSidebarType !== 'none') {
        ...
        if (currentSidebarType === 'ai-assistant') {
          ...
          if (sidebarStack.length === 0 || sidebarStack[0] !== defaultSidebarType) {
  ...
  const workplaceItems = ['dashboard', 'daily-roster', 'meeting-services', 'space-requests', 'issue-reports', 'announcements', 'visitors', 'deliveries', 'surveys', 'devices', 'calendars'];
  ...
  const settingsItems = ['settings-organization', 'settings-offices', 'settings-themes', 'settings-integrations', 'settings-people', 'settings-groups', 'settings-roles', 'settings-calendars', 'settings-event-audit-logs', 'settings-hybrid-work-policies', 'settings-access', 'settings-daily-roster', 'settings-announcements', 'settings-surveys', 'settings-devices', 'settings-amenities', 'settings-stickers', 'settings-workplace-services', 'settings-notifications', 'settings-billing', 'settings-priority-support'];
  ...
  const handleOpenSidebar = (newSidebarType: 'ai-assistant' | 'meeting-details' | 'create-meeting' | 'filters') => {
    ...
    if (hasDefaultSidebar) {
      ...
      // Add new sidebar type if not already at top
      if (newStack.length === 0 || newStack[newStack.length - 1] !== newSidebarType) {
        newStack.push(newSidebarType);
      }
    ...
    switch (page) {
      ...
      case "tickets":
        return [
          ...
          { icon: faCircleInfo, text: "learn about the automated ticket system." },
        ];
      case "assignment":
        return [
          { icon: faMagicWandSparkles, text: "learn how AI seat assignment works." },
          { icon: faCircleInfo, text: "understand what factors determine seat suggestions." },
  ...
  const generateAiResponse = (prompt: string, page: string): string => {
    ...
    if (promptLower.includes('catering') || promptLower.includes('add catering')) {
    ...
    if (promptLower.includes('notification') || promptLower.includes('attention')) {
    ...
    if (promptLower.includes('priority') || promptLower.includes('high priority')) {
    ...
    if (onAiAssistantMessagesUpdate) {
      ...
      setTimeout(() => {
        ...
        if (isCateringPrompt) {
          ...
          const robinContent = "If you click a meeting in the grid to the left I'll add some of this info for you. Or pick from the list below:";
  ...
  const hasSidebarContent = (sidebarContent && currentSidebarType !== 'none' && currentSidebarType !== 'ai-assistant' && currentSidebarType !== 'meeting-details' && currentSidebarType !== 'create-meeting' && currentSidebarType !== 'filters') ||
    (hasDefaultSidebar && currentSidebarType !== 'ai-assistant' && currentSidebarType !== 'meeting-details' && currentSidebarType !== 'create-meeting' && currentSidebarType !== 'filters');
  const showAiButton = currentSidebarType !== 'ai-assistant';

  // Show back button if there's something to go back to in the stack
  const showBackButton = (currentSidebarType === 'ai-assistant' || currentSidebarType === 'meeting-details' || currentSidebarType === 'create-meeting' || currentSidebarType === 'filters') && (
    sidebarStack.length > 1 || // Multiple items in stack
    (hasDefaultSidebar && sidebarStack.length === 1 && currentSidebarType !== (getDefaultSidebarType() as SidebarType)) // On default sidebar page but not showing default
  );

  // Always show close button for AI assistant, meeting details, create meeting, and filters sidebars
  const showCloseButton = (currentSidebarType === 'ai-assistant' || currentSidebarType === 'meeting-details' || currentSidebarType === 'create-meeting' || currentSidebarType === 'filters');

  // Navigation item with tooltip for collapsed state
  const NavItem = React.forwardRef<HTMLDivElement, {
    view: string;
    ...
    const baseContent = (
      <div
        ref={ref}
        className={isNavCollapsed ? getNavItemClassCollapsed(view) : getNavItemClass(view)}
    ...
    const wrappedContent = wrapperComponent ? wrapperComponent(baseContent) : baseContent;
  ...
  const WorkplaceFlyout = () => {
    ...
    return (
      ...
      >
        <div className="px-3 py-1 text-xs text-gray-400 border-b border-gray-700 flex-shrink-0">Workplace</div>
        <div className="overflow-y-auto scrollbar-overlay py-2 flex-1 min-h-0">
          {[
            ...
            <div
              key={view}
              className={`px-4 py-2 hover:bg-gray-700 cursor-pointer text-sm whitespace-nowrap ${view === currentView ? 'text-white' : 'text-gray-300'
                }`}
              style={{ fontSize: '12px', ...(view === currentView ? { backgroundColor: '#17559C' } : {}) }}
              onClick={() => {
                onNavigate(view);
  ...
  const SettingsFlyout = () => {
    ...
    return (
      ...
      >
        <div className="px-3 py-1 text-xs text-gray-400 border-b border-gray-700 flex-shrink-0">Settings</div>
        <div className="overflow-y-auto scrollbar-overlay py-2 flex-1 min-h-0">
          ...
          {[
            { view: 'settings-hybrid-work-policies', label: 'Hybrid work policies' },
          ...
          {[
            { view: 'settings-workplace-services', label: 'Workplace services' },
          ...
          <div
            className={`px-4 py-2 hover:bg-gray-700 cursor-pointer text-sm whitespace-nowrap ${'settings-priority-support' === currentView ? 'text-white' : 'text-gray-300'
              }`}
            style={{ fontSize: '12px', ...('settings-priority-support' === currentView ? { backgroundColor: '#17559C' } : {}) }}
            onClick={() => {
  ...
  return (
    <div className="h-screen bg-gray-50 flex overflow-hidden max-w-full">
      ...
      <div className={`max-[600px]:w-9 ${isNavCollapsed ? 'w-18' : 'w-[200px]'} text-white h-screen flex flex-col transition-all duration-300 flex-shrink-0`} style={{ backgroundColor: '#131A23' }}>
        {/* Fixed Header */}
        <div className="p-4 max-[600px]:p-2 flex-shrink-0 relative">
          {/* Wayne Ent. Branding at Top */}
          <div className={`flex items-center space-x-2 ${isNavCollapsed ? 'justify-center' : ''} max-[600px]:justify-center`}>
            <div className="w-8 h-8 max-[600px]:w-6 max-[600px]:h-6 bg-white rounded flex items-center justify-center">
              <span className="text-[#1e3a8a] text-sm max-[600px]:text-xs font-semibold">W</span>
            </div>
            {!isNavCollapsed && (
              <span className="text-xl font-semibold text-white max-[600px]:hidden">Wayne Ent.</span>
            )}
          </div>
          ...
          {onNavCollapsedChange && (
            ...
            >
              {isNavCollapsed ? (
                <FontAwesomeIcon icon={faChevronRight} className="text-white" style={{ fontSize: '7px' }} />
              ) : (
                <FontAwesomeIcon icon={faChevronLeft} className="text-white" style={{ fontSize: '7px' }} />
              )}
            </button>
          )}
        </div>
        ...
        <div ref={navScrollContainerRef} className="flex-1 overflow-y-auto scrollbar-hide">
          <nav className="p-4 max-[600px]:p-1 space-y-1 max-[600px]:space-y-0">
            {/* Primary Navigation Items - Before Workplace */}
            <NavItem
              view="map"
              icon={<FontAwesomeIcon icon={faMapMarkerAlt} className="w-3.5 h-3.5" />}
              label="Map"
            ...
            <NavItem
              view="meeting-spaces"
              icon={<FontAwesomeIcon icon={faDoorOpen} className="w-3.5 h-3.5" />}
            ...
            {isNavCollapsed ? (
              <div
                ...
                className={isWorkplaceItemSelected ?
                ...
                onClick={() => !isWorkplaceItemSelected && onNavigate('dashboard')}
                onMouseEnter={() => {
                  setShowWorkplaceFlyout(true);
                  setShowSettingsFlyout(false);
                }}
              >
                <FontAwesomeIcon icon={faCity} className="w-3.5 h-3.5 max-[600px]:w-2.5 max-[600px]:h-2.5" />
              </div>
            ) : (
              <div className="space-y-1">
                <div
                  className={`flex items-center justify-between ${isWorkplaceItemSelected && !isWorkplaceExpanded ? 'text-white' : 'text-gray-300 hover:text-white'} px-3 py-2 cursor-pointer rounded`}
                  onClick={() => {
                  ...
                  style={isWorkplaceItemSelected && !isWorkplaceExpanded ? { backgroundColor: '#17559C' } : {}}
                >
                  <div className="flex items-center space-x-3">
                ...
                {isWorkplaceExpanded && (
                  <div className="-mx-4 space-y-1 px-4" style={{ backgroundColor: '#000C17' }}>
                    <div
                      className={getWorkplaceSubItemClass('dashboard')}
                      onClick={() => onNavigate('dashboard')}
                      style={{ fontSize: '12px', ...('dashboard' === currentView ? getActiveStyle() : {}) }}
                    >
                      Dashboard
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('daily-roster' === currentView ? getActiveStyle() : {}) }}
                    >
                      Daily roster
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('meeting-services' === currentView ? getActiveStyle() : {}) }}
                    >
                      Meeting services
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('space-requests' === currentView ? getActiveStyle() : {}) }}
                    >
                      Space requests
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('issue-reports' === currentView ? getActiveStyle() : {}) }}
                    >
                      Issue reports
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('announcements' === currentView ? getActiveStyle() : {}) }}
                    >
                      Announcements
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('visitors' === currentView ? getActiveStyle() : {}) }}
                    >
                      Visitors
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('deliveries' === currentView ? getActiveStyle() : {}) }}
                    >
                      Deliveries
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('surveys' === currentView ? getActiveStyle() : {}) }}
                    >
                      Surveys
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('devices' === currentView ? getActiveStyle() : {}) }}
                    >
                      Devices
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('calendars' === currentView ? getActiveStyle() : {}) }}
                    >
                      Calendars
                    </div>
                  </div>
            ...
            <NavItem
              view="space-planning"
              icon={<FontAwesomeIcon icon={faTableCells} className="w-3.5 h-3.5" />}
            ...
            <NavItem
              view="analytics"
              icon={<FontAwesomeIcon icon={faChartLine} className="w-3.5 h-3.5" />}
              label="Analytics"
            ...
            {isNavCollapsed ? (
              <div
                ref={settingsItemRef}
                className={isSettingsItemSelected ?
                  "text-white w-10 h-10 max-[600px]:w-full max-[600px]:h-8 rounded flex items-center justify-center" :
                  "text-gray-300 hover:text-white w-10 h-10 max-[600px]:w-full max-[600px]:h-8 cursor-pointer rounded hover:bg-gray-800 flex items-center justify-center"
                }
                style={isSettingsItemSelected ? getActiveStyle() : {}}
                onClick={() => !isSettingsItemSelected && onNavigate('settings-organization')}
                onMouseEnter={() => {
                  setShowSettingsFlyout(true);
                  setShowWorkplaceFlyout(false);
                }}
              >
                <FontAwesomeIcon icon={faCog} className="w-3.5 h-3.5 max-[600px]:w-2.5 max-[600px]:h-2.5" />
              </div>
            ) : (
              <div className="space-y-1">
                <div
                  className={`flex items-center justify-between ${isSettingsItemSelected && !isSettingsExpanded ? 'text-white' : 'text-gray-300 hover:text-white'} px-3 py-2 cursor-pointer rounded`}
                  onClick={() => {
                  ...
                  style={isSettingsItemSelected && !isSettingsExpanded ? { backgroundColor: '#17559C' } : {}}
                >
                  <div className="flex items-center space-x-3">
                ...
                {isSettingsExpanded && (
                  <div className="-mx-4 space-y-1 px-4" style={{ backgroundColor: '#000C17' }}>
                    {/* Group 1: Organization, Offices, Themes, Integrations */}
                    <div
                      className={getSettingsSubItemClass('settings-organization')}
                      onClick={() => onNavigate('settings-organization')}
                      style={{ fontSize: '12px', ...('settings-organization' === currentView ? getActiveStyle() : {}) }}
                    >
                      Organization
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('settings-offices' === currentView ? getActiveStyle() : {}) }}
                    >
                      Offices
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('settings-themes' === currentView ? getActiveStyle() : {}) }}
                    >
                      Themes
                    </div>
                    <div
                      className={getSettingsSubItemClass('settings-integrations')}
                      onClick={() => onNavigate('settings-integrations')}
                      style={{ fontSize: '12px', ...('settings-integrations' === currentView ? getActiveStyle() : {}) }}
                    >
                      Integrations
                    </div>
                    ...
                    <div
                      ...
                      style={{ fontSize: '12px', ...('settings-people' === currentView ? getActiveStyle() : {}) }}
                    >
                      People
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('settings-groups' === currentView ? getActiveStyle() : {}) }}
                    >
                      Groups
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('settings-roles' === currentView ? getActiveStyle() : {}) }}
                    >
                      Roles
                    </div>
                    ...
                    <div
                      ...
                      style={{ fontSize: '12px', ...('settings-calendars' === currentView ? getActiveStyle() : {}) }}
                    >
                      Calendars
                    </div>
                    <div
                      className={getSettingsSubItemClass('settings-event-audit-logs')}
                      onClick={() => onNavigate('settings-event-audit-logs')}
                      style={{ fontSize: '12px', ...('settings-event-audit-logs' === currentView ? getActiveStyle() : {}) }}
                    >
                      Event audit logs
                    </div>
                    ...
                    {/* Group 4: Hybrid work policies, Access, Daily roster, Announcements, Surveys */}
                    <div
                      className={getSettingsSubItemClass('settings-hybrid-work-policies')}
                      onClick={() => onNavigate('settings-hybrid-work-policies')}
                      style={{ fontSize: '12px', ...('settings-hybrid-work-policies' === currentView ? getActiveStyle() : {}) }}
                    >
                    ...
                    <div
                      ...
                      style={{ fontSize: '12px', ...('settings-access' === currentView ? getActiveStyle() : {}) }}
                    >
                      Access
                    </div>
                    <div
                      className={getSettingsSubItemClass('settings-daily-roster')}
                      onClick={() => onNavigate('settings-daily-roster')}
                      style={{ fontSize: '12px', ...('settings-daily-roster' === currentView ? getActiveStyle() : {}) }}
                    >
                      Daily roster
                    </div>
                    <div
                      className={getSettingsSubItemClass('settings-announcements')}
                      onClick={() => onNavigate('settings-announcements')}
                      style={{ fontSize: '12px', ...('settings-announcements' === currentView ? getActiveStyle() : {}) }}
                    >
                      Announcements
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('settings-surveys' === currentView ? getActiveStyle() : {}) }}
                    >
                      Surveys
                    </div>
                    ...
                    <div
                      ...
                      style={{ fontSize: '12px', ...('settings-devices' === currentView ? getActiveStyle() : {}) }}
                    >
                      Devices
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('settings-amenities' === currentView ? getActiveStyle() : {}) }}
                    >
                      Amenities
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('settings-stickers' === currentView ? getActiveStyle() : {}) }}
                    >
                      Stickers
                    </div>
                    ...
                    {/* Group 6: Workplace services, Notifications, Billing */}
                    <div
                      className={getSettingsSubItemClass('settings-workplace-services')}
                      onClick={() => onNavigate('settings-workplace-services')}
                      style={{ fontSize: '12px', ...('settings-workplace-services' === currentView ? getActiveStyle() : {}) }}
                    >
                      Workplace services
                    </div>
                    <div
                      className={getSettingsSubItemClass('settings-notifications')}
                      onClick={() => onNavigate('settings-notifications')}
                      style={{ fontSize: '12px', ...('settings-notifications' === currentView ? getActiveStyle() : {}) }}
                    >
                      Notifications
                    </div>
                    <div
                      ...
                      style={{ fontSize: '12px', ...('settings-billing' === currentView ? getActiveStyle() : {}) }}
                    >
                      Billing
                    </div>
                    ...
                    {/* Group 7: Priority support */}
                    <div
                      className={getSettingsSubItemClass('settings-priority-support')}
                      onClick={() => onNavigate('settings-priority-support')}
                      style={{ fontSize: '12px', ...('settings-priority-support' === currentView ? getActiveStyle() : {}) }}
                    >
                      Priority support
                    </div>
                  </div>
        ...
        <div className="p-4 max-[600px]:p-1 border-t border-gray-700 space-y-1 max-[600px]:space-y-0 flex-shrink-0">
          ...
          <NavItem
            view="support"
            icon={<FontAwesomeIcon icon={faQuestionCircle} className="w-3.5 h-3.5" />}
            label="Support"
            onClick={() => { }}
            onMouseEnter={() => {
              setShowWorkplaceFlyout(false);
                ...
                return (
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div>
                          <Popover open={showSupportPopover} onOpenChange={setShowSupportPopover}>
                            <PopoverTrigger asChild>
                              {children}
                            </PopoverTrigger>
                            <PopoverContent className="w-80 p-0" align="start" side="right" sideOffset={16}>
                              <div className="p-4 border-b border-gray-200">
                                <h3 style={{ fontSize: '16px', fontWeight: 500 }}>Get Support</h3>
                                <p className="text-gray-500 mt-1" style={{ fontSize: '14px', fontWeight: 400 }}>Choose a support option to get help</p>
                              </div>
                              ...
                              <div className="p-3 space-y-2">
                                ...
                                >
                                  <div className="flex items-start space-x-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <p style={{ fontSize: '14px', fontWeight: 500 }} className="text-gray-900">Technical Support</p>
                                      <p className="text-gray-500 mt-0.5" style={{ fontSize: '12px', fontWeight: 400 }}>Issues with room bookings, devices, or system errors</p>
                                    </div>
                                  </div>
                                </button>
                                ...
                                >
                                  <div className="flex items-start space-x-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <p style={{ fontSize: '14px', fontWeight: 500 }} className="text-gray-900">Account & Billing</p>
                                      <p className="text-gray-500 mt-0.5" style={{ fontSize: '12px', fontWeight: 400 }}>Questions about your account or billing</p>
                                    </div>
                                  </div>
                                </button>
                                ...
                                >
                                  <div className="flex items-start space-x-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <p style={{ fontSize: '14px', fontWeight: 500 }} className="text-gray-900">General Inquiry</p>
                                      <p className="text-gray-500 mt-0.5" style={{ fontSize: '12px', fontWeight: 400 }}>Other questions or workplace requests</p>
                                    </div>
                                  </div>
                                </button>
                              </div>
                              ...
                              <div className="p-3 border-t border-gray-200 bg-gray-50">
                                <p className="text-gray-500" style={{ fontSize: '12px', fontWeight: 400 }}>
                                  Need immediate help? Call us at <span className="text-blue-600" style={{ fontWeight: 500 }}>1-800-WAYNE-ENT</span>
                                </p>
                              </div>
                            </PopoverContent>
                          </Popover>
          ...
          <NavItem
            view="feedback"
            icon={<FontAwesomeIcon icon={faComment} className="w-3.5 h-3.5 max-[600px]:w-2.5 max-[600px]:h-2.5" />}
            label="Feedback"
            onClick={() => { }}
            onMouseEnter={() => {
              setShowWorkplaceFlyout(false);
                      ...
                      <TooltipTrigger asChild>
                        <div>
                          <Popover open={showFeedbackPopover} onOpenChange={setShowFeedbackPopover}>
                            ...
                            <PopoverContent className="w-96 p-0" align="start" side="right" sideOffset={16}>
                              <div className="p-4 border-b border-gray-200">
                                <h3 style={{ fontSize: '16px', fontWeight: 500 }}>Send Feedback</h3>
                                <p className="text-gray-500 mt-1" style={{ fontSize: '14px', fontWeight: 400 }}>Help us improve your experience</p>
                              </div>

                              <div className="p-4 space-y-4">
                                <div className="space-y-2">
                                  <label htmlFor="feedback-type" className="text-gray-700" style={{ fontSize: '14px', fontWeight: 500 }}>
                                    Feedback Type
                                  </label>
                                ...
                                <div className="space-y-2">
                                  <label htmlFor="feedback-subject" className="text-gray-700" style={{ fontSize: '14px', fontWeight: 500 }}>
                                    Subject
                                  </label>
                                  <Input
                                    id="feedback-subject"
                                ...
                                <div className="space-y-2">
                                  <label htmlFor="feedback-message" className="text-gray-700" style={{ fontSize: '14px', fontWeight: 500 }}>
                                    Message
                                  </label>
                                  <Textarea
                                    id="feedback-message"
                  ...
                  return (
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <div>
                            <Popover onOpenChange={(open) => !open && onNotificationPopoverClose()}>
                              <PopoverTrigger asChild>
                                {children}
                              </PopoverTrigger>
                              <PopoverContent className="w-80 p-0" align="start" side="right" sideOffset={16}>
      ...
      <div className="flex-1 flex flex-col h-screen min-w-0 overflow-hidden">
        ...
        <header className={`bg-white border-b border-gray-200 ${meetingSpacesViewMode === 'month' && selectedMonthViewRoom ? '' : 'shadow-sm'
          } flex-shrink-0 max-[600px]:mr-0 relative z-10 ${currentSidebarType === 'ai-assistant' || currentSidebarType === 'meeting-details' || currentSidebarType === 'create-meeting' || currentSidebarType === 'room-details' ? 'mr-96' :
            hasSidebarContent ? 'mr-96' : currentView === 'map' ? '' : ''
          }`}>
          ...
          <div className="px-6 max-[600px]:px-3 py-4 max-[600px]:py-2 pt-[10px] pr-[24px] pb-[10px] pl-[24px] max-[600px]:pt-2 max-[600px]:pr-3 max-[600px]:pb-2 max-[600px]:pl-3">
            ...
            <div className="flex items-center justify-between mb-3">
              <h1 className="text-gray-900 max-[599px]:text-[22px]" style={{ fontSize: '30px', fontWeight: 400 }}>{pageTitle}</h1>
                ...
                {showAiButton && (
                  <div className="flex items-center bg-gradient-to-r from-primary to-blue-600 hover:from-primary/90 hover:to-blue-600/90 rounded-[4px] transition-all">
                    ...
                    <Button
                      ...
                      style={{ fontSize: '14px', fontWeight: 500 }}
                    >
                      <FontAwesomeIcon icon={faMagicWandSparkles} className="h-4 w-4 max-[599px]:mr-0 mr-2" />
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          className="h-[36px] w-[36px] p-0 bg-transparent hover:bg-transparent text-white rounded-l-none rounded-r-[4px] max-[599px]:hidden"
                        >
                      ...
                      >
                        ...
                        <div className="bg-white p-[4px] rounded-[4px]">
                          {/* Preset Prompts */}
                          {getPageSpecificPrompts(currentView).map((prompt, index) => (
                            <DropdownMenuItem
                              key={index}
                              onClick={() => handleAiPromptSelect(prompt.text)}
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <Input
                                ...
                                onChange={(e) => setAiDropdownInput(e.target.value)}
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter') {
                                    e.preventDefault();
            ...
            {(currentView === 'map' || !hideLocationPicker || !hideDatePicker || currentView === 'meeting-spaces') && (
              <div className="flex items-center space-x-3">
                ...
                {currentView === 'map' ? (
                  <div className="flex items-center space-x-2">
                    ...
                    <Select value={selectedLocation} onValueChange={setSelectedLocation}>
                      <SelectTrigger hideChevron className="h-9 border border-[#868686] bg-white text-[rgba(0,0,0,0.95)] hover:text-[rgba(0,0,0,0.95)] flex items-center justify-between space-x-2 pl-3 pr-3 max-[599px]:pl-2 max-[599px]:pr-2 w-auto max-w-[150px] max-[599px]:max-w-none max-[599px]:w-9 whitespace-nowrap cursor-pointer">
                        <div className="flex items-center space-x-2 max-[599px]:space-x-0">
                          <FontAwesomeIcon icon={faMapMarkerAlt} className="w-4 h-4 text-[rgba(0,0,0,0.55)]" />
                          <span className="max-[599px]:hidden"><SelectValue placeholder="Select location" className="text-[rgba(0,0,0,0.95)]" /></span>
                        </div>
                      </SelectTrigger>
                      <SelectContent className="max-[599px]:w-[calc(100vw-24px)]">
                        <SelectItem value="54 State St.">54 State St.</SelectItem>
                        <SelectItem value="100 Main St.">100 Main St.</SelectItem>
                        <SelectItem value="200 Broadway">200 Broadway</SelectItem>
                        <SelectItem value="500 Park Ave.">500 Park Ave.</SelectItem>
                      </SelectContent>
                    </Select>

                    {/* Floor Picker */}
                    <Select value={selectedFloor} onValueChange={onFloorChange}>
                      <SelectTrigger hideChevron className="h-9 border border-[#868686] bg-white text-[rgba(0,0,0,0.95)] hover:text-[rgba(0,0,0,0.95)] flex items-center justify-between space-x-2 w-auto max-w-[150px] max-[599px]:max-w-none max-[599px]:w-9 whitespace-nowrap pl-3 pr-3 max-[599px]:pl-2 max-[599px]:pr-2 cursor-pointer">
                        <div className="flex items-center space-x-2 max-[599px]:space-x-0">
                          <FontAwesomeIcon icon={faLayerGroup} className="w-4 h-4 text-[rgba(0,0,0,0.55)]" />
                          <span className="text-[rgba(0,0,0,0.95)] max-[599px]:hidden" style={{ fontSize: '14px', fontWeight: 400 }}>Floor {selectedFloor}</span>
                        </div>
                      </SelectTrigger>
                      <SelectContent className="max-[599px]:w-[calc(100vw-24px)]">
                    ...
                    <Select value={selectedDate} onValueChange={setSelectedDate}>
                      <SelectTrigger hideChevron className="h-9 border border-[#868686] bg-white text-[rgba(0,0,0,0.95)] hover:text-[rgba(0,0,0,0.95)] flex items-center justify-between space-x-2 pl-3 pr-3 max-[599px]:pl-2 max-[599px]:pr-2 w-auto max-[599px]:w-9 whitespace-nowrap cursor-pointer">
                        <div className="flex items-center space-x-2 max-[599px]:space-x-0">
                          <FontAwesomeIcon icon={faCalendar} className="w-4 h-4 text-[rgba(0,0,0,0.55)]" />
                          <span className="max-[599px]:hidden text-[rgba(0,0,0,0.95)]" style={{ fontSize: '14px', fontWeight: 400 }}>{getDateDisplayWithoutYear(selectedDate)}</span>
                        </div>
                      </SelectTrigger>
                    ...
                    <div className="flex items-center space-x-2">
                      <Select value={selectedTime} onValueChange={setSelectedTime}>
                        <SelectTrigger hideChevron className="h-9 border border-[#868686] bg-white text-[rgba(0,0,0,0.95)] hover:text-[rgba(0,0,0,0.95)] flex items-center justify-between space-x-2 w-auto max-w-[150px] max-[599px]:max-w-none max-[599px]:w-9 whitespace-nowrap pl-3 pr-3 max-[599px]:pl-2 max-[599px]:pr-2 cursor-pointer">
                          <div className="flex items-center space-x-2 max-[599px]:space-x-0">
                            <FontAwesomeIcon icon={faClock} className="w-4 h-4 text-[rgba(0,0,0,0.55)]" />
                            <span className="text-[rgba(0,0,0,0.95)] max-[599px]:hidden" style={{ fontSize: '14px', fontWeight: 400 }}>{selectedTime}</span>
                          </div>
                        </SelectTrigger>
                        <SelectContent className="h-[200px] overflow-y-auto max-[599px]:w-[calc(100vw-24px)]">
                      ...
                      {!showEndTime && (
                        <span className="text-sm text-gray-600" style={{ fontSize: '14px', fontWeight: 400 }}>EST</span>
                      )}
                      ...
                      {!showEndTime ? (
                        ...
                        <>
                          {/* Dash separator */}
                          <span className="text-[rgba(0,0,0,0.55)]" style={{ fontSize: '14px', fontWeight: 400 }}>-</span>

                          <Select
                            value={selectedEndTime}
                            onValueChange={setSelectedEndTime}
                          >
                            <SelectTrigger hideChevron className="h-9 border border-[#868686] bg-white text-[rgba(0,0,0,0.95)] hover:text-[rgba(0,0,0,0.95)] flex items-center justify-between space-x-2 w-auto max-w-[150px] whitespace-nowrap pl-3 pr-3 cursor-pointer">
                              <div className="flex items-center space-x-2">
                                <FontAwesomeIcon icon={faClock} className="w-4 h-4 text-[rgba(0,0,0,0.55)]" />
                                {selectedEndTime ? (
                                  <span className="text-[rgba(0,0,0,0.95)]" style={{ fontSize: '14px', fontWeight: 400 }}>{selectedEndTime}</span>
                                ) : (
                                  <span className="text-[rgba(0,0,0,0.55)]" style={{ fontSize: '14px', fontWeight: 400 }}>End time</span>
                                )}
                              </div>
                            </SelectTrigger>
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        ...
                        >
                          <span style={{ fontSize: '14px', fontWeight: 400 }}>Edit floor</span>
                          <FontAwesomeIcon icon={faChevronDown} className="w-4 h-4 text-[rgba(0,0,0,0.55)]" />
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end" className="w-72">
                        <DropdownMenuItem className="cursor-pointer" style={{ fontSize: '14px', fontWeight: 400 }}>
                  ...
                  <>
                    ...
                    {showFloorPicker && (
                      <div className="flex items-center space-x-2">
                        ...
                        <Select value={selectedFloor} onValueChange={onFloorChange}>
                          <SelectTrigger hideChevron className="w-20 h-9 border-none shadow-none text-gray-900 hover:text-gray-900 font-medium cursor-pointer">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                  ...
                  <>
                    ...
                    <div className="flex items-center bg-gray-100 rounded-md p-0.5 h-9">
                      ...
                      <button
                        ...
                        className={`px-3 h-8 rounded transition-colors flex items-center cursor-pointer ${meetingSpacesViewMode === 'day'
                      ...
                      <button
                        ...
                        className={`px-3 h-8 rounded transition-colors flex items-center cursor-pointer ${meetingSpacesViewMode === 'week'
                          ? 'bg-white text-gray-900 shadow-sm'
                          : 'text-gray-600 hover:text-gray-900'
                          }`}
                        style={{ fontSize: '13px', fontWeight: 500 }}
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <button
                            className={`px-3 h-8 rounded transition-colors flex items-center gap-1 cursor-pointer ${meetingSpacesViewMode === 'month'
                              ? 'bg-white text-gray-900 shadow-sm'
                              : 'text-gray-600 hover:text-gray-900'
                              }`}
                            style={{ fontSize: '13px', fontWeight: 500 }}
                        ...
                        <DropdownMenuContent align="end" className="w-64">
                          <div className="px-2 py-1.5" style={{ fontSize: '12px', fontWeight: 500, color: 'rgba(0,0,0,0.5)' }}>
                          ...
                          {allRooms && allRooms.length > 0 ? (
                            ...
                            <div className="px-2 py-1.5 text-gray-500" style={{ fontSize: '14px' }}>
                              No rooms available
                            </div>
                          )}
                        </DropdownMenuContent>
          ...
          {pageTabs && pageTabs.length > 0 && onPageTabChange && activePageTab && (
        ...
        {currentView === 'map' ? (
          ...
          <div className={`flex-1 flex min-w-0 ${fullViewport ? "overflow-hidden" : "overflow-y-auto"} max-[600px]:mr-0 ${currentSidebarType === 'ai-assistant' || currentSidebarType === 'meeting-details' || currentSidebarType === 'create-meeting' || currentSidebarType === 'filters' || currentSidebarType === 'room-details' || currentSidebarType === 'service-ticket' ? 'mr-96' : ''
            }`}>
            {/* Map Content Area */}
          ...
          <div className={`flex-1 min-w-0 ${fullViewport ? "overflow-hidden" : "overflow-y-auto"} max-[600px]:mr-0 ${currentSidebarType === 'service-ticket' ? 'mr-96' : ''
            }`}>
            <MeetingServicesPage
          ...
          <div className={`flex-1 min-w-0 ${fullViewport ? "overflow-hidden" : "overflow-y-auto"} max-[600px]:mr-0 ${currentSidebarType === 'ai-assistant' || currentSidebarType === 'meeting-details' || currentSidebarType === 'create-meeting' || currentSidebarType === 'filters' || currentSidebarType === 'room-details' || currentSidebarType === 'service-ticket' ? 'mr-96' :
            hasSidebarContent ? 'mr-96' : ''
            }`}>
            {children}
          </div>
        )}
      ...
      {(hasSidebarContent || currentSidebarType === 'ai-assistant' || currentSidebarType === 'meeting-details' || currentSidebarType === 'create-meeting' || currentSidebarType === 'filters' || currentSidebarType === 'room-details' || currentSidebarType === 'service-ticket') && (
        <div className="fixed top-0 right-0 h-full w-96 bg-white border-l border-gray-200 shadow-lg z-50 flex flex-col">
          ...
          {currentSidebarType === 'create-meeting' && meetingCreationContext && allRooms && (
          ...
          {currentSidebarType !== 'ai-assistant' && currentSidebarType !== 'meeting-details' && currentSidebarType !== 'create-meeting' && currentSidebarType !== 'filters' && currentSidebarType !== 'room-details' && currentSidebarType !== 'service-ticket' && hasDefaultSidebar && defaultSidebarContent}
          {currentSidebarType !== 'ai-assistant' && currentSidebarType !== 'meeting-details' && currentSidebarType !== 'create-meeting' && currentSidebarType !== 'filters' && currentSidebarType !== 'room-details' && currentSidebarType !== 'service-ticket' && !hasDefaultSidebar && sidebarContent}
        </div>
      )}
