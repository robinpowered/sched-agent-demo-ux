...
export const EditMeetingForm = React.forwardRef<
  HTMLFormElement,
...
>(
  (
    ...
    const generateAttendeeList = (meeting: Meeting): string[] => {
      ...
      const organizerEmail = `${meeting.organizer
        .toLowerCase()
        .replace(" ", ".")}@wayne-enterprises.com`;
      ...
      for (let i = 1; i < attendeeCount; i++) {
        ...
        attendeeEmails.push(
          `${firstName.toLowerCase()}.${lastName.toLowerCase()}@wayne-enterprises.com`
        );
      }
    ...
    const availableRooms = allRooms.filter(
      (room) => room.meetings && room.meetings.length > 0
    );
    ...
    const selectedRoomObjects = selectedRooms
      .map(
        (roomId) =>
          availableRooms.find((room) => room.id === roomId) ||
          allRooms.find((room) => room.id === roomId)
      )
      .filter(Boolean) as Room[];

    // Get available rooms for adding (excluding already selected ones)
    const availableRoomsForAdd = availableRooms.filter(
      (room) =>
    ...
    const availableUsersForAdd = allUsers.filter(
      (user) => !attendeeList.includes(user.email)
    );
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setStartTime(parseFloat(value))}
                    >
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setEndTime(parseFloat(value))}
                    >
                      <SelectTrigger className="mt-1 w-full">
                        <SelectValue placeholder="Select time" />
                      </SelectTrigger>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {selectedRoomObjects.map((room) => (
                <Card
                  key={room.id}
                  className="overflow-hidden border-gray-200 rounded-[4px]"
                >
            ...
            {selectedRooms.length === 0 && (
              <p className="text-sm text-red-600">
                Please select at least one room for the meeting.
              </p>
            )}
          </div>
