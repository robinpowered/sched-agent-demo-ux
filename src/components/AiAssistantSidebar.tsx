...
import { CateringOrderPreview, CateringOrderDetails, CateringOrderItem } from "./CateringOrderPreview";
...
// Import shared types
import { Message, ChatSession, Meeting, Room, MeetingRequirements } from '../types';
            ...
            <div className="flex flex-col gap-[4px] w-full">
              ...
              <div className="h-[20px] relative w-full">
                ...
                <div className="absolute flex flex-col font-['Brown_LL',_sans-serif] justify-center left-[18px] text-[#1c1c1c] text-[12px] top-[10px] translate-y-[-50%]">
                  <p className="leading-[20px] overflow-ellipsis overflow-hidden whitespace-pre font-medium">Good fit</p>
                </div>
              </div>
              
              {/* Room items */}
              {goodFitRooms.map(room => (
                <div key={room.id} className="flex flex-col font-['Brown_LL',_sans-serif] text-[10px] w-full">
                  <div className="flex flex-col justify-center overflow-ellipsis overflow-hidden relative text-[#1c1c1c] text-nowrap w-full">
                    <p className="leading-[20px] overflow-ellipsis overflow-hidden font-medium">{room.name}</p>
                  </div>
                  <div className="flex flex-col justify-center relative text-[#515151] w-full">
                    <p className="leading-[15px] font-normal">
                      With a capacity of {room.capacity}, it's a good fit for the {room.attendees} invitee{room.attendees > 1 ? 's' : ''}.
                    </p>
                  </div>
                </div>
              ))}
          ...
          {poorFitRooms.length > 0 && (
            <div className="flex flex-col gap-[4px] w-full">
              ...
              <div className="h-[20px] relative w-full">
                ...
                <div className="absolute flex flex-col font-['Brown_LL',_sans-serif] justify-center left-[18px] text-[#1c1c1c] text-[12px] top-[10px] translate-y-[-50%]">
                  <p className="leading-[20px] overflow-ellipsis overflow-hidden whitespace-pre font-medium">Poor fit</p>
                </div>
              </div>
...
export function AiAssistantSidebar({
  ...
  const [thinkingProgress, setThinkingProgress] = useState<Record<string, { visibleLines: number; totalLines: number; isComplete: boolean }>>({});
  ...
  const [localCateringOrderDetails, setLocalCateringOrderDetails] = useState<CateringOrderDetails>({
    items: [],
    totalCost: 0
  });
  const cateringOrderDetails = externalCateringOrderDetails || localCateringOrderDetails;
  const updateCateringOrder = onCateringOrderUpdate || setLocalCateringOrderDetails;
  const [isCateringPreviewOpen, setIsCateringPreviewOpen] = useState(true);
  const [localCateringOrderSubmitted, setLocalCateringOrderSubmitted] = useState(false);
  const cateringOrderSubmitted = externalCateringOrderSubmitted !== undefined ? externalCateringOrderSubmitted : localCateringOrderSubmitted;
  const setCateringOrderSubmitted = onCateringOrderSubmittedChange || setLocalCateringOrderSubmitted;
  ...
  const cateringTicketNumber = externalCateringTicketNumber || localCateringTicketNumber;
  ...
  React.useEffect(() => {
    ...
    thinkingMessages.forEach(message => {
      const lines = message.thinkingText!.split('\n').filter(line => line.trim());
  ...
  React.useEffect(() => {
    ...
    if (messages.length >= 2) {
      ...
      if (
        ...
        userMessage.content.toLowerCase().includes('book a room for my team') &&
        aiMessage?.sender === 'assistant' &&
        aiMessage.content.includes('The more you tell me about your meeting')
      ) {
        // Auto-populate the input with example booking after a delay
        setTimeout(() => {
          setInputValue(
  ...
  React.useEffect(() => {
    const thinkingMessages = messages.filter(m => m.isThinking && m.thinkingText && !m.isPaused);
  ...
  React.useEffect(() => {
    messages.forEach((message) => {
      ...
      const dotsTimeout = setTimeout(() => {
        ...
        setMessages(prev => 
          prev.map(m => 
            m.id === message.id ? { ...m, isTyping: false, typedContent: '' } : m
          )
        );
        ...
        const typeNextChar = () => {
          if (charIndex <= fullContent.length) {
            setMessages(prev => 
              prev.map(m => 
                m.id === message.id ? { ...m, typedContent: fullContent.substring(0, charIndex) } : m
              )
            );
            charIndex++;
  ...
  const shouldShowHistoryButton =
    ...
    (hasStartedChat || chatHistory.length > 1);

  // Handler for confirming menu order
  const handleConfirmMenuOrder = (items: MenuItem[], restaurantName: string) => {
    ...
    setTimeout(() => {
      const aiResponse: Message = {
        ...
        content: `Great! I've added those items to your catering order. Your total is $${totalCost.toFixed(2)} from ${restaurantName}. Is there anything else you'd like to add?`,
        sender: "assistant",
        isTyping: true,
        agentType: "ezcater",
      };
  ...
  const handleCuisineSelect = (cuisineText: string) => {
    ...
    if (isCateringItemsRequest) {
      ...
      setTimeout(() => {
        const aiResponse: Message = {
          id: (Date.now() + 1).toString(),
          content: `Great! I've found some nearby restaurants that can deliver ${items.join(' and ')} for your meeting.`,
          sender: "assistant",
          isTyping: true,
          agentType: "ezcater",
  ...
  const handleCancelCateringOrder = () => {
    ...
    setTimeout(() => {
      const aiResponse: Message = {
        id: (Date.now() + 1).toString(),
        content: "No problem! I've canceled your catering order. Let me know if you'd like to start a new one.",
        sender: "assistant",
      };
      const finalMessages = [...newMessages, aiResponse];
      setMessages(finalMessages);
  ...
  const handleUpdateCateringQuantity = (itemName: string, newQuantity: number) => {
    const updatedItems = cateringOrderDetails.items.map(item => 
      item.name === itemName ? { ...item, quantity: newQuantity } : item
    ).filter(item => item.quantity > 0); // Remove items with 0 quantity
    
    const newTotalCost = updatedItems.reduce((sum, item) => sum + (item.price * item.quantity), 0);
  ...
  const handleRemoveCateringItem = (itemName: string) => {
    const updatedItems = cateringOrderDetails.items.filter(item => item.name !== itemName);
    const newTotalCost = updatedItems.reduce((sum, item) => sum + (item.price * item.quantity), 0);
  ...
  const handleAddCateringItem = (item: CateringOrderItem) => {
    ...
    const existingItem = cateringOrderDetails.items.find(i => i.name === item.name);
    ...
    const newTotalCost = updatedItems.reduce((sum, i) => sum + (i.price * i.quantity), 0);
  ...
  const handleSubmitCateringOrder = () => {
    const orderSummary = `Submit catering order from ${cateringOrderDetails.restaurant} for $${cateringOrderDetails.totalCost.toFixed(2)}`;
    ...
    setTimeout(() => {
      ...
      // Show success toast
      toast.success('Catering order submitted successfully!', { duration: 3000 });
    }, 800);
  };
  ...
  const handleSendMessage = (messageContent?: string) => {
    ...
    const isSubmitOrderRequest = 
      (input.includes("submit") && (input.includes("order") || input.includes("catering"))) ||
      (input.includes("place") && (input.includes("order") || input.includes("catering"))) ||
      (input.includes("confirm") && (input.includes("order") || input.includes("catering"))) ||
    ...
    const hasCoffee = /coffee/i.test(input);
    const hasPastries = /(pastries|pastry)/i.test(input);
    const hasOtherFood = /(sandwiches|lunch|breakfast|snacks|drinks|beverages|food)/i.test(input);
    ...
    const hasPeopleCount = /(\d+)\s*(?:people|person|attendees?|participants?)/i.test(content);
    const hasTime = /(\d+)(?::(\d+))?\s*(am|pm)/i.test(content);
    const hasAmenities = /video|projector|whiteboard|audio|screen|tv|conferencing/i.test(content);
    ...
    if (isDetailedBookingRequest) {
      ...
      setTimeout(() => {
        ...
        const messagesWithIntermediate = [...updatedMessages, intermediateMessage];
        ...
        setTimeout(() => {
          ...
          const goodFitRooms = rooms?.filter(room => {
            // Must be available
            if (room.status !== 'available') return false;
            
            // Good fit: capacity is within attendees to attendees + 3
            if (room.capacity < requirements.attendees || room.capacity > requirements.attendees + 3) {
              return false;
            }
            
            // Must have all required features
            if (requirements.features.length > 0) {
              const hasAllFeatures = requirements.features.every(requiredFeature => 
            ...
            const hasConflict = room.meetings.some(existingMeeting => {
              ...
              return !(meetingEnd <= existingStart || requirements.startTime >= existingEnd);
            });
          ...
          const poorFitRooms = rooms?.filter(room => {
            // Must be available
            if (room.status !== 'available') return false;
            ...
            const missingFeatures = requirements.features.length > 0 && 
          ...
          const featureText = requirements.features.length > 0 
            ? `, and have ${requirements.features.map(f => {
          ...
          const poorFitRoomData = poorFitRooms.map(room => {
            ...
            if (capacityTooSmall) {
              ...
              const missingFeaturesList = requirements.features.filter(requiredFeature => 
                !room.features.some(roomFeature => 
                  roomFeature.toLowerCase().includes(requiredFeature.toLowerCase()) ||
                  requiredFeature.toLowerCase().includes(roomFeature.toLowerCase())
                )
              ...
              if (missingFeaturesList.length > 0) {
                const formattedFeatures = missingFeaturesList.map(f => {
                  const lower = f.toLowerCase();
                  if (lower === 'video conf') return 'video-conferencing';
                  return lower;
                }).join(', ');
          ...
          const firstThinking: Message = {
            ...
            sender: "assistant",
            isThinking: true,
            thinkingText: `• Looking for spaces that...\n  fit at least ${requirements.attendees} ${requirements.attendees === 1 ? 'person' : 'people'}, are available at ${formatTime(requirements.startTime)} today${featureText}.\n• Found ${goodFitCount} suitable space${goodFitCount !== 1 ? 's' : ''}\n  There ${goodFitCount !== 1 ? 'are' : 'is'} ${goodFitCount} good ${goodFitCount !== 1 ? 'choices' : 'choice'} so I should ${goodFitCount > 1 ? 'ask the user which one they would prefer' : goodFitCount === 1 ? 'recommend this space' : 'look for alternative options'}.\n  [ROOM_LIST]`,
            thinkingRoomData: {
              goodFitRooms: goodFitRoomData,
              poorFitRooms: poorFitRoomData
            }
          };
          const messagesWithFirstThinking = [...messagesWithIntermediate, firstThinking];
      ...
      setTimeout(() => {
        ...
        // Auto-populate input if this was a room booking request
        if (shouldAutoPopulate) {
          setTimeout(() => {
            setInputValue(
              "I need to book a team check-in for 4 people at 2pm today with video-conferencing",
            );
          }, 500); // Small delay after AI response appears
        }
  ...
  const handleEditMessage = (messageId: string) => {
    ...
    const activeThinkingMessages = messages.filter(m => 
      m.isThinking && 
      !m.isPaused && 
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const poorFitRoomData = poorFitRooms.map(room => {
            ...
            let reason = '';
            if (capacityTooSmall) {
              reason = `With a capacity of ${room.capacity}, this space is too small for ${requirements.attendees} attendee${requirements.attendees > 1 ? 's' : ''}.`;
            } else if (capacityTooLarge) {
              reason = `With a capacity of ${room.capacity}, this space is much larger than needed for ${requirements.attendees} attendee${requirements.attendees > 1 ? 's' : ''}.`;
            } else {
...
• You need space for ${requirements.attendees} attendee${requirements.attendees > 1 ? 's' : ''}
• Meeting duration: ${requirements.duration === 0.5 ? '30 minutes' : requirements.duration === 1 ? '1 hour' : `${requirements.duration} hours`}
• Start time: ${formatTime(requirements.startTime)}${featureText}

Checking room availability
• Found ${goodFitCount} room${goodFitCount !== 1 ? 's' : ''} that match your criteria
...
• Confirming no scheduling conflicts`;
          ...
          const messagesWithThinking = [...messagesWithIntermediate, thinkingMessage];
          setMessages(messagesWithThinking);
          onMessagesUpdate?.(messagesWithThinking);

          // Add final response with room suggestions after thinking
          setTimeout(() => {
            const finalResponse = goodFitCount > 0
              ? `I found ${goodFitCount} room${goodFitCount !== 1 ? 's' : ''} that ${goodFitCount === 1 ? 'fits' : 'fit'} your needs. ${goodFitCount === 1 ? 'This space is' : 'These spaces are'} available at ${formatTime(requirements.startTime)} and ${goodFitCount === 1 ? 'has' : 'have'} the capacity and features you need.`
  ...
  const parseMeetingRequirements = (input: string) => {
    ...
    if (inputLower.includes("standup") || inputLower.includes("stand up") || inputLower.includes("daily")) {
      title = "Team Standup";
      type = "standup";
    } else if (inputLower.includes("presentation") || inputLower.includes("demo")) {
      title = "Presentation";
      type = "presentation";
      ...
      title = "Interview";
      type = "interview";
    } else if (inputLower.includes("review") || inputLower.includes("retrospective") || inputLower.includes("retro")) {
      title = "Review Meeting";
      type = "review";
    } else if (inputLower.includes("brainstorm") || inputLower.includes("ideation")) {
      title = "Brainstorming Session";
      type = "brainstorm";
    } else if (inputLower.includes("1:1") || inputLower.includes("one on one") || inputLower.includes("1-on-1")) {
      title = "1:1 Meeting";
      type = "1:1";
    } else if (inputLower.includes("check-in") || inputLower.includes("check in") || inputLower.includes("sync")) {
      title = "Team Check-in";
      type = "check-in";
  ...
  const getAiResponse = (userInput: string): string => {
    ...
    // Check if user provided enough details for a room search
    const hasPeopleCount = /(\d+)\s*(?:people|person|attendees?|participants?)/i.test(userInput);
    const hasTime = /(\d+)(?::(\d+))?\s*(am|pm)/i.test(userInput);
    const hasAmenities = /video|projector|whiteboard|audio|screen|tv|conferencing/i.test(input);
  ...
  const parseMessageContent = (content: string): (string | React.ReactElement)[] | string => {
      ...
      {!hasStartedChat ? (
            ...
            <div className="box-border content-stretch flex flex-col gap-4 items-center justify-center p-4 relative w-full">
                    ...
                    <div className="basis-0 content-stretch flex flex-col gap-2 grow items-start min-h-px min-w-px relative shrink-0">
                      ...
                      <div className="flex flex-col font-normal justify-center leading-[0] not-italic relative shrink-0 text-[#1c1c1c] text-[14px] w-full">
                        <p className="leading-[22px]">
                          I can help you manage your events and
                          spaces.
                        </p>
                      </div>
                    </div>
              ...
              <div className="content-stretch flex flex-col gap-2 items-start relative shrink-0 w-full">
                ...
                <div className="content-stretch flex flex-col gap-3 items-start relative shrink-0 w-full">
                  {getPageSpecificPrompts(currentPage).map(
                    (prompt, index) => (
                      <button
                        key={index}
                        onClick={() =>
                        ...
                        <div className="relative size-full">
                          <div className="box-border content-stretch flex gap-3 items-start px-4 py-3 relative w-full">
                            <div className="flex-shrink-0 mt-[2px]">
                              <FontAwesomeIcon icon={prompt.icon} className="h-4 w-4 text-primary" />
                            </div>
        ...
        <>
              ...
              return turns.map((turn, turnIndex) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      minHeight: isCurrentTurn && isCurrentTurnFullHeight ? '100%' : undefined,
                      display: isCurrentTurn && isCurrentTurnFullHeight ? 'flex' : undefined,
                      flexDirection: isCurrentTurn && isCurrentTurnFullHeight ? 'column' : undefined,
                      justifyContent: isCurrentTurn && isCurrentTurnFullHeight ? 'flex-start' : undefined,
                    }}
                  >
                    {turn.map((message) => (
                      <div
                  ...
                  <div
                    ...
                    onMouseLeave={() => message.sender === "user" && setHoveredMessageId(null)}
                  >
                    {/* Edit button - appears on hover to the left of user messages */}
                    {message.sender === "user" && hoveredMessageId === message.id && editingMessageId !== message.id && (
                      <button
                    ...
                    {message.sender === "assistant" && !message.isThinking && (
                      <div className="w-8 h-8 flex-shrink-0 mt-1">
                        {message.agentType === 'ezcater' ? <EzCaterAvatar /> : <Avatar />}
                      </div>
                    )}
                    ...
                    <div className="flex flex-col space-y-1 max-w-[320px]">
                      ...
                      {message.sender === "assistant" && !message.isThinking && (
                        <div className="text-gray-900">
                          {message.agentType === 'ezcater' ? 'ezCater Agent' : 'Robin AI Assistant'}
                        </div>
                      )}
                      ...
                      <div className="flex flex-col space-y-2">
                        ...
                        >
                          ...
                          <span className={`text-sm ${thinkingProgress[message.id]?.isComplete || message.isPaused ? '' : 'italic'}`}>
                            {message.isPaused ? 'Paused' : (thinkingProgress[message.id]?.isComplete ? 'See thinking' : message.content)}
                          </span>
                          {!thinkingProgress[message.id]?.isComplete && !message.isPaused && (
                          ...
                          <div className="mt-2 ml-6">
                            <div className="relative p-[0px]">
                              ...
                              <div className="space-y-3">
                                {message.thinkingText && thinkingProgress[message.id] && (() => {
                                  const allLines = message.thinkingText.split('\n')
                                    .filter(line => line.trim())
                                    .slice(0, thinkingProgress[message.id].visibleLines);
                                  ...
                                  const parsedSteps: Array<{ 
                                    main: string; 
                                    hasBullet: boolean;
                                    nested: Array<{ text: string; isBulleted: boolean }> 
                                  }> = [];
                                  let currentStep: { 
                                    main: string; 
                                    hasBullet: boolean;
                                  ...
                                  allLines.forEach(line => {
                                    if (line.startsWith('• ')) {
                                      ...
                                      currentStep = { main: line.substring(2), hasBullet: true, nested: [] };
                                    } else if (line.startsWith('  • ')) {
                                      // Nested bulleted item
                                      if (currentStep) {
                                        currentStep.nested.push({ text: line.substring(4), isBulleted: true });
                                      }
                                    } else if (line.startsWith('  ')) {
                                      // Nested plain text
                                      if (currentStep) {
                                        currentStep.nested.push({ text: line.trim(), isBulleted: false });
                                      }
                                    } else {
                                      ...
                                      currentStep = { main: line, hasBullet: false, nested: [] };
                                    }
                                  });
                                  ...
                                  if (parsedSteps.length > 1) {
                                    ...
                                    for (let i = 0; i < parsedSteps.length - 1; i++) {
                                      ...
                                      if (step.nested.length > 0) {
                                        ...
                                        lineHeight += step.nested.length * 20 + (step.nested.length - 1) * 4;
                                      }
                                    ...
                                    <>
                                      ...
                                      {parsedSteps.map((step, index) => (
                                        <div key={index} className="space-y-1">
                                          ...
                                          {/* Nested items */}
                                          {step.nested.length > 0 && (
                                            <div className="ml-[28px] space-y-1">
                                              {step.nested.map((nested, nestedIndex) => {
                                                // Check if this is the room list placeholder
                                                if (nested.text === '[ROOM_LIST]' && message.thinkingRoomData) {
                                                  return (
                                                    <div key={nestedIndex}>
                                                      <ThinkingRoomList
                                                        goodFitRooms={message.thinkingRoomData.goodFitRooms}
                                                        poorFitRooms={message.thinkingRoomData.poorFitRooms}
                                                      />
                                                    </div>
                                                  );
                                                }
                                                ...
                                                return (
                                                  <div key={nestedIndex} className="flex items-start gap-2 text-[12px] font-normal text-[#6c6c6c] leading-[20px]">
                                                    {nested.isBulleted && <span className="flex-shrink-0">•</span>}
                                                    <span className={nested.isBulleted ? '' : 'block'}>
                                                      {nested.text}
                                                    </span>
                                                  </div>
                                                );
                      ...
                      editingMessageId === message.id ? (
                        ...
                        >
                          ...
                          {message.sender === "assistant" && 
                           message.isTyping && 
                           !completedTypingMessages.current.has(message.id) && 
                          ...
                          >
                            {message.typedContent !== undefined 
                                ? parseMessageContent(message.typedContent)
                    ...
                    message.restaurantName && (
                      <div className="w-full mt-3">
                        <CateringMenuCard
                          ...
                          onConfirmOrder={(items) => handleConfirmMenuOrder(items, message.restaurantName!)}
                        />
                      </div>
                    )}
                  ...
                  {message.sender === "assistant" &&
                    message.showMeetingListWidget && (() => {
                      ...
                      const upcomingMeetings: Array<{ meeting: Meeting; room: Room }> = [];
                      ...
                      upcomingMeetings.sort((a, b) => a.meeting.startTime - b.meeting.startTime);
                      const nextFiveMeetings = upcomingMeetings.slice(0, 5);
                      
                      return nextFiveMeetings.length > 0 ? (
                        <div className="w-full mt-3">
                          <MeetingListWidget
                            meetings={nextFiveMeetings}
                            onSelectMeeting={handleMeetingSelectFromWidget}
                          />
                        </div>
                      ) : null;
                    })()}
          ...
          <div className="p-4 border-t border-gray-200 flex-shrink-0">
            ...
            {messages.some(m => m.agentType === 'ezcater') && !cateringOrderSubmitted && (() => {
              // Get available menu items from the most recent menu message
              const menuMessage = messages.slice().reverse().find(m => m.showCateringMenu && m.menuItems);
              const availableMenuItems = menuMessage?.menuItems?.map(item => ({
                ...item,
              ...
              return (
                <div className="border border-[#868686] rounded-t-lg border-b-0 overflow-hidden bg-white mb-0">
                  <CateringOrderPreview
                    ...
                    onToggle={() => setIsCateringPreviewOpen(!isCateringPreviewOpen)}
            ...
            <div 
              className="relative"
              style={{
                borderTopLeftRadius: messages.some(m => m.agentType === 'ezcater') && !cateringOrderSubmitted ? '0' : undefined,
                borderTopRightRadius: messages.some(m => m.agentType === 'ezcater') && !cateringOrderSubmitted ? '0' : undefined,
              }}
            >
              <Textarea
                ref={inputRef}
