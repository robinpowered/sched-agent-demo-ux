...
function MonthView({
  ...
  if (!room) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center py-12 text-gray-500">No room selected</div>
      </div>
    );
  }
  ...
  const isMeetingCurrent = (dayDate: number, meeting: Meeting) => {
    ...
    const meetingEndTime = meeting.startTime + meeting.duration;
    return (
      meeting.startTime <= currentTimeFloat && currentTimeFloat < meetingEndTime
    );
  };
  ...
  const getMonthDays = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      if (dayOfWeek >= 1 && dayOfWeek <= 5) {
        ...
        dayMeetings = room.meetings
          .filter((_, meetingIndex) => {
              ...
              <TooltipProvider key={day.date}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div
                      className={`rounded p-1 relative flex flex-col cursor-pointer transition-all hover:shadow-sm ${
                        isHighActivity
                      ...
                      {dayMeetings.length > 0 && (
                        <div className="space-y-0.5 overflow-y-auto scrollbar-overlay relative z-10 flex-1">
                          {dayMeetings.map((meeting, idx) => {
                            ...
                            const isCurrent = isMeetingCurrent(
                              day.date,
                              meeting
                            );
                            ...
                            return (
                              <button
                                key={idx}
                                className={`w-full p-1 text-white rounded cursor-pointer transition-colors text-left ${
                                  isPast
                  ...
                  {isHighActivity && (
                    <TooltipContent>
                      <p>
                        This office has an unusually high number of meetings
                        today
                      </p>
                    </TooltipContent>
                  )}
  ...
  const getMeetingsForDay = (room: Room, dayIndex: number): Meeting[] => {
    ...
    switch (pattern) {
      case 0: // Busy early week, lighter later
        if (dayIndex === 0)
          return meetings.slice(0, Math.ceil(meetings.length * 0.4));
        if (dayIndex === 1)
          return meetings.slice(
            Math.ceil(meetings.length * 0.4),
            Math.ceil(meetings.length * 0.7)
          );
        if (dayIndex === 2)
          return meetings.slice(
            Math.ceil(meetings.length * 0.7),
            Math.ceil(meetings.length * 0.85)
          );
        if (dayIndex === 3)
          return meetings.slice(
            Math.ceil(meetings.length * 0.85),
            Math.ceil(meetings.length * 0.95)
          );
        if (dayIndex === 4)
          return meetings.slice(Math.ceil(meetings.length * 0.95));
        break;
      ...
      case 2: // Busy mid-week
        ...
        if (dayIndex === 1)
          return meetings.slice(1, Math.ceil(meetings.length * 0.3));
        if (dayIndex === 2)
          return meetings.slice(
            Math.ceil(meetings.length * 0.3),
            Math.ceil(meetings.length * 0.7)
          );
        if (dayIndex === 3)
          return meetings.slice(
      ...
      case 3: // Busy Friday (today)
        ...
        if (dayIndex === 3)
          return meetings.slice(3, Math.ceil(meetings.length * 0.4));
        if (dayIndex === 4)
          return meetings.slice(Math.ceil(meetings.length * 0.4));
        break;
  ...
  const RoomRow = ({ room }: { room: Room }) => {
    ...
    return (
      <div className="flex border-b border-[#D6D6D6] min-h-[64px]">
        ...
        <div
          className={`w-60 max-[600px]:w-32 ${
            isOffline ? "bg-[#FFD6D6]" : "bg-gray-50"
          } flex items-start pl-[16px] pr-2 max-[600px]:px-2 py-3 flex-shrink-0`}
              ...
              <RoomPreviewPopover room={room}>
                ...
                >
                  <div className="flex items-center gap-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1 min-w-0">
                      <div
                        className={`text-sm max-[600px]:text-xs text-[rgba(0,0,0,0.95)] truncate ${
                          !isRestricted && !room.requestOnly ? "ml-4" : ""
                        }`}
            ...
            return (
              <div
                ...
                className={`p-2 flex flex-col gap-1.5 ${
                  dayIndex < 4 ? "border-r border-[#D6D6D6]" : ""
                } ${
                  isOffline ? "bg-[#FFD6D6]" : isToday ? "bg-[#f0f6ff]" : ""
                }`}
              >
                {dayMeetings.map((meeting) => {
                  ...
                  return (
                    <button
                      key={meeting.id}
                      className={`w-full p-1.5 text-white rounded cursor-pointer transition-colors text-left flex-shrink-0 ${
                        isPast
                          ? "bg-gray-400 hover:bg-gray-500"
                          : "bg-[#2774C1] hover:bg-[#1e5a8a]"
                      }`}
            ...
            {weekdays.map((day, index) => (
              <div
                ...
                className={`flex items-center justify-center ${
                  index < 4 ? "border-r border-[#D6D6D6]" : ""
                } ${index === todayIndex ? "bg-[#f0f6ff]" : ""}`}
                style={{
                  fontSize: "12px",
                  fontWeight: 500,
            ...
            <div className="flex">
              ...
              >
                <div className="flex items-center">
                  <div
                    className="text-[rgba(0,0,0,0.95)] max-[600px]:text-xs"
                ...
                {isPinnedExpanded ? (
                  <FontAwesomeIcon
                    icon={faChevronDown}
                    className="w-4 h-4 max-[600px]:w-3 max-[600px]:h-3 text-[rgba(0,0,0,0.55)]"
                  />
                ) : (
                  <FontAwesomeIcon
                    icon={faChevronRight}
...
export function MeetingSpacesPage({
  ...
  const [confirmationState, setConfirmationState] = useState<
    "idle" | "checking-capacity" | "checking-permissions" | "ready"
  >("idle");
  ...
  const [localSyncingMeetings, setLocalSyncingMeetings] = useState<Set<string>>(
    new Set()
  );

  // Combine local syncing meetings with those passed from parent (AI bookings)
  const allSyncingMeetings = new Set([
    ...localSyncingMeetings,
    ...(syncingMeetings || []),
  ]);
  ...
  const isMeetingCurrent = (meeting: Meeting) => {
    ...
    return (
      meeting.startTime <= currentHourFloat &&
      currentHourFloat < meeting.startTime + meeting.duration
    );
  };
  ...
  const handleMeetingPreviewUpdate = (
  ...
  const handleDrop = (
    ...
    const hasConflictAtTarget = targetRoom.meetings.some((existingMeeting) => {
      ...
      // Check for overlap
      const hasOverlap = !(
        meetingEnd <= existingStart || meetingStart >= existingEnd
      );
    ...
    if (hasConflictAtTarget) {
      ...
      const conflictedMeeting = targetRoom.meetings.find((existingMeeting) => {
        ...
        return !(
          targetTimeSlot >= endTime || newEndTime <= existingMeeting.startTime
        );
      });
  ...
  const handleConfirmConflictMove = () => {
    ...
    const { meeting: autoMovedMeeting, targetTimeSlot: autoMovedTimeSlot } =
      conflictResolution.autoMovedMeeting;
    const {
      meeting: draggedMeeting,
      targetRoomId,
      targetTimeSlot: draggedTimeSlot,
    } = conflictResolution.draggedMeeting;
    const selectedOption =
      conflictResolution.availableOptions[
        conflictResolution.currentOptionIndex
      ];
    ...
    setTimeout(() => {
      ...
      const draggedRoomName =
        allRooms.find((room) => room.id === targetRoomId)?.name ||
        "the selected room";
    ...
    setConflictResolution((prev) =>
      prev
        ? {
            ...prev,
            currentOptionIndex:
              prev.currentOptionIndex >= prev.availableOptions.length - 1
                ? 0
                : prev.currentOptionIndex + 1,
          }
        : null
  ...
  const handleGrabHandleDragStart = (
    e: React.DragEvent,
    meeting: any,
    roomId: string
  ) => {
    ...
    const meetingBlock = e.currentTarget.closest(
      ".meeting-block"
    ) as HTMLElement;
    if (meetingBlock) {
      ...
      const meetingColor =
        meetingBlock.className.match(/(bg-\w+-\d+)/)?.[0] || "bg-[#2774C1]";
  ...
  const getMeetingColor = (meeting: Meeting, index: number) => {
    ...
    const isCurrentMeeting =
      meeting.startTime <= currentHourFloat &&
      meetingEndTime > currentHourFloat;
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) =>
      handleTimeLineDrag(e, gridElement);
  ...
  const isRoomCheckedIn = (room: Room) => {
    ...
    return room.meetings.some((meeting) => {
      ...
      const meetingEnd = meeting.startTime + meeting.duration;
      const isCurrentMeeting =
        currentHourFloat >= meetingStart && currentHourFloat < meetingEnd;
  ...
  const canRoomAccommodateMeeting = (
    room: Room,
    meeting: Meeting,
  ...
  const canPlaceMeeting = (
    room: Room,
    meeting: { startTime: number; duration: number; id?: string }
  ) => {
    const meetingStart = meeting.startTime;
    const meetingEnd = meeting.startTime + meeting.duration;
  ...
  const relocateMeetingsFromOfflineRoom = (roomsArray: Room[]) => {
    ...
    const confRoomBIndex = updatedRoomsArray.findIndex(
      (room) => room.id === "conf-b"
    );
    if (confRoomBIndex !== -1) {
      ...
      {meetingSpacesViewMode === "week" ? (
        ...
        <div className="flex h-full flex-col relative">
          ...
          <div className="flex border-b border-[#D6D6D6] shadow-sm bg-white relative z-20">
            ...
            <div className="flex-1 bg-white overflow-hidden h-full">
              <div
                className="grid gap-0 h-full transition-transform duration-[1000ms] ease-[cubic-bezier(0.25,0.1,0.25,1)]"
                style={{
                  ...
                  return (
                    <div
                      ...
                      className={`text-center ${colSpan} ${
                        hourIndex !== 0 ? "border-l border-[#D6D6D6]" : ""
                    ...
                    >
                      {selectedTimezones.map((timezone, tzIndex) => {
                        ...
                        const baseTimezoneOffset = getTimezoneOffset(
                          selectedTimezones[0]
                        );
                        const currentTimezoneOffset =
                          getTimezoneOffset(timezone);
                        const offset =
                          currentTimezoneOffset - baseTimezoneOffset;
                ...
                {normalizedPinnedRoomIds.length > 0 && (
                  <div>
                    ...
                    >
                      <div className="flex items-center">
                        <div
                          className="text-[rgba(0,0,0,0.95)] max-[600px]:text-xs ml-4"
                          style={{ fontSize: "14px", fontWeight: 700 }}
                        >
                          Pinned Spaces
                        </div>
                    ...
                    {isPinnedExpanded && (
                      <div>
                        {normalizedPinnedRoomIds.map((roomId) => {
                          ...
                          const roomHasInsufficientCapacity =
                            hasInsufficientCapacity(room);
                          const hasExcessCapacity =
                            draggedMeeting &&
                            room.capacity - draggedMeeting.meeting.attendees >
                              4;
                          ...
                          return (
                            <div
                              key={room.id}
                              className={`border-b border-[#D6D6D6] flex items-center transition-colors pl-2 pr-2 max-[600px]:px-2 ${sidebarRowBgColor} ${
                                compactView ? "h-10" : "h-16"
                              } group`}
                            >
                              <div className="flex items-center w-full">
                                <div className="flex-1 min-w-0 overflow-hidden">
                    ...
                    {expandedFloors.has(floor) && (
                      <div>
                        {groupedRooms[floor].map((room) => {
                          const roomHasInsufficientCapacity =
                            hasInsufficientCapacity(room);
                          const isPinned = normalizedPinnedRoomIds.includes(
                            room.id
                          );
                          const hasExcessCapacity =
                            draggedMeeting &&
            ...
            <div className="flex-1 bg-gray-50 relative overflow-hidden">
              ...
              >
                ...
                {timeLinePosition !== null && (
                  ...
                  >
                    <div
                      className={`absolute -top-2 -left-3 w-6 h-4 bg-[#E81C1C] rounded-sm flex items-center justify-center ${
                        isDraggingTime
                    ...
                    <div
                      className={`absolute inset-0 ${
                        isDraggingTime ? "cursor-grabbing" : "cursor-grab"
                      }`}
                    />
                  </div>
                )}
                <div className="min-h-full">
                      ...
                      {isPinnedExpanded &&
                        pinnedRoomIds.map((roomId) => {
                          ...
                          return (
                            <div
                              key={room.id}
                              className={`relative border-b border-[#D6D6D6] ${rowBgColor} ${
                                compactView ? "h-10" : "h-16"
                              }`}
                            >
                              ...
                              >
                                {timeSlotValues.map((timeValue, index) => {
                                  const hasConflictHere = hasConflict(
                                    room,
                                    timeValue
                                  );
                                  const isHovered =
                                    hoveredTimeSlot?.roomId === room.id &&
                                    hoveredTimeSlot?.timeSlot === timeValue;
                                  const cannotDrop =
                                    hasConflictHere ||
                                    meetingCreationContext ||
                                    isRestricted;
                              ...
                              <TooltipProvider>
                                {room.meetings
                                  .filter((meeting) => {
                                    if (
                                      pendingMove &&
                                      meeting.id === pendingMove.meeting.id
                                    )
                                      return false;
                                    if (conflictResolution) {
                                      if (
                                  ...
                                  .map((meeting, meetingIndex) => {
                                    ...
                                    const isMyMeeting =
                                      ...
                                      meeting.aiCreated;
                                    const shouldDimMeeting =
                                      spotlightMyEvents && !isMyMeeting;

                                    const shouldDimByDuration =
                                      activeFilters?.duration !== "any" &&
                                      activeFilters?.duration &&
                                      meeting.duration * 60 <
                                        parseInt(activeFilters.duration);
                                    const isSyncing = allSyncingMeetings.has(
                                      meeting.id
                                    );

                                    // Fade all regular meetings to 10% when special interactions are active (after drop, not during drag)
                                    const shouldFadeForInteraction =
                                      aiMeetingPreview ||
                                      pendingMove ||
                                      conflictResolution;
                                    ...
                                    const meetingElement = (
                                      <div
                                        key={meeting.id}
                                        className={`meeting-block absolute rounded px-2 flex items-center transition-all duration-200 ${
                                          compactView
                                            ? "top-1 bottom-1"
                                            : "top-2 bottom-2"
                                        } ${
                                          isSyncing
                                            ...
                                            : `${getMeetingColor(
                                                ...
                                                meeting.pendingApproval
                                                  ? "text-[#2774C1] border-2 border-[#2774C1]"
                                                  : isMeetingFuture(meeting)
                                                  ? "text-[#2774C1]"
                                                  : "text-white"
                                              } cursor-pointer hover:opacity-90`
                                        } ${
                                          shouldFadeForInteraction
                                            ? "opacity-25"
                                            : shouldDimMeeting ||
                                              shouldDimByDuration
                                            ? "opacity-25"
                                        ...
                                        onMouseLeave={() =>
                                          !isSyncing && setHoveredMeeting(null)
                                        }
                                        onClick={() =>
                                          !isSyncing &&
                                          handleMeetingClick(meeting, room)
                                        }
                                      >
                                        <div
                                        ...
                                        >
                                          <div className="flex items-center gap-1.5">
                                            ...
                                            {meeting.pendingApproval && (
                                              <FontAwesomeIcon
                                                icon={faLock}
                                                className="w-3 h-3 text-[#2774C1] flex-shrink-0"
                                              />
                                            )}
                                          </div>
                                          {!compactView && (
                                    ...
                                    return isSyncing ? (
                                      <Tooltip key={meeting.id}>
                                        ...
                                        <TooltipContent>
                                          <p>
                                            Meeting is syncing. This may take a
                                            moment.
                                          </p>
                                        </TooltipContent>
                                      </Tooltip>
                              ...
                              {aiMeetingPreview &&
                                aiMeetingPreview.roomId === room.id &&
                                (() => {
                                  const startCol =
                                    aiMeetingPreview.startTime * 2;
                                  const widthCols =
                                    aiMeetingPreview.duration * 2;
                                  const leftPercent = (startCol / 48) * 100;
                                  const widthPercent = (widthCols / 48) * 100;

                                  return (
                                    <div
                                      className={`absolute bg-gradient-to-r from-primary to-blue-600 rounded px-2 text-white flex items-center z-50 ${
                                        compactView
                                          ? "top-1 bottom-1"
                                          : "top-2 bottom-2"
                                      }`}
                  ...
                  {floors.map((floor) => (
                    <React.Fragment key={`floor-${floor}`}>
                      ...
                      {expandedFloors.has(floor) &&
                        groupedRooms[floor].map((room, roomIndex) => {
                          const roomHasInsufficientCapacity =
                            hasInsufficientCapacity(room);
                          ...
                          return (
                            ...
                            >
                                ...
                                {room.meetings
                                  ...
                                  .map((meeting, meetingIndex) => {
                                    ...
                                    const shouldDimByDuration =
                                      ...
                                      activeFilters?.duration &&
                                      meeting.duration * 60 <
                                        parseInt(activeFilters.duration); // Convert meeting duration to minutes
                                    const isSyncing = allSyncingMeetings.has(
                                      meeting.id
                                    );
                                    ...
                                    const meetingElement = (
                                      ...
                                      >
                                        ...
                                        <div
                                          className={`flex-shrink-0 transition-all duration-200 ${
                                            isSyncing || isMeetingPast(meeting)
                                              ? "opacity-0 w-0 mr-0"
                                              : `cursor-grab active:cursor-grabbing ${
                                                  hoveredMeeting === meeting.id
                                                    ? "opacity-100 w-4 mr-2"
                              ...
                              {pendingMove &&
                                pendingMove.targetRoomId === room.id &&
                                (() => {
                                  const startCol =
                                    pendingMove.targetTimeSlot * 2;
                                  const widthCols =
                                    pendingMove.meeting.duration * 2;
                                  const leftPercent = (startCol / 48) * 100;
                                  const widthPercent = (widthCols / 48) * 100;

                                  // Apply spotlight opacity for pending moves too
                                  const isMyPendingMeeting =
                                    pendingMove.meeting.organizer ===
                                      currentUser ||
                                    pendingMove.meeting.aiCreated;
                                  const shouldDimPendingMeeting =
                                    spotlightMyEvents && !isMyPendingMeeting;

                                  // Apply duration filter opacity for pending moves
                                  const shouldDimPendingByDuration =
                                    activeFilters?.duration !== "any" &&
                                    activeFilters?.duration &&
                                    pendingMove.meeting.duration * 60 <
                                      parseInt(activeFilters.duration);
                                  ...
                                  return (
                                    <Popover open={true} modal={false}>
                                      <PopoverTrigger asChild>
                                        <div
                                          className={`absolute ${meetingColor} rounded px-2 ${
                                            ...
                                            shouldDimPendingMeeting ||
                                            shouldDimPendingByDuration
                                              ? "opacity-25"
                                              : ""
                                          }`}
                                          style={{
                                          ...
                                          <div className="space-y-3">
                                            ...
                                            <div className="flex flex-col">
                                              <p className="font-['Brown_LL',_sans-serif] leading-[20px] not-italic text-[#6c6c6c] text-[12px]">
                                                {formatTimeSlot(
                                                  pendingMove.targetTimeSlot
                                                )}{" "}
                                                -{" "}
                                                {formatTimeSlot(
                                                  pendingMove.targetTimeSlot +
                                                    pendingMove.meeting.duration
                                                )}
                                              </p>
                                              <p className="font-['Brown_LL',_sans-serif] leading-[22px] not-italic text-[#1c1c1c] text-[14px] font-medium">
                                                {pendingMove.meeting.title}
                                              </p>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-3 w-full">
                                                  ...
                                                  const originalRoom =
                                                    updatedRooms.find((r) =>
                                                      r.meetings.some(
                                                        (m) =>
                                                          m.id ===
                                                          pendingMove.meeting.id
                                                      )
                                                    );
                                                  if (!originalRoom)
                                                    return null;
                                                  const amenitiesCount =
                                                    originalRoom.features
                                                      ?.length || 0;
                                                  return (
                                                      ...
                                                      <div className="flex flex-wrap gap-[8px] items-center">
                                                        ...
                                                        {amenitiesCount > 0 && (
                                                          <div className="flex items-center gap-[4px]">
                                                            <span className="font-['Brown_LL',_sans-serif] leading-[20px] not-italic text-[#6c6c6c] text-[12px]">
                                                              •
                                                            </span>
                                                            <Tooltip>
                                                              <TooltipTrigger
                                              ...
                                              <FontAwesomeIcon
                                                icon={faArrowRight}
                                                className="w-4 h-4 text-[#6c6c6c] shrink-0"
                                              />
                                              ...
                                              return (
                                                <div className="bg-[#FFFCF0] border border-[#FFEBAB] rounded-[4px] p-3">
                                                  <div className="flex gap-2">
                                                    <FontAwesomeIcon
                                                      icon={faLock}
                                                      className="w-4 h-4 text-[#8A5006] flex-shrink-0 mt-0.5"
                                                    />
                                                    <p className="font-['Brown_LL',_sans-serif] leading-[20px] not-italic text-[#8A5006] text-[13px]">
                                                      Using this space requires
                                                      approval. Moving a meeting
                                                      here will send a request
                              ...
                              {conflictResolution &&
                                ...
                                (() => {
                                  ...
                                  const isMyConflictMeeting =
                                    meeting.organizer === currentUser ||
                                    meeting.aiCreated;
                                  const shouldDimConflictMeeting =
                                    spotlightMyEvents && !isMyConflictMeeting;

                                  // Apply duration filter opacity for conflict meetings
                                  const shouldDimConflictByDuration =
                                    activeFilters?.duration !== "any" &&
                                    activeFilters?.duration &&
                                    meeting.duration * 60 <
                                      parseInt(activeFilters.duration);
                                  ...
                                  return (
                                    <div
                                      className={`absolute bg-[#C20E14] rounded px-2 text-white flex items-center transition-all duration-200 ${
                                        compactView
                                          ? "top-1 bottom-1"
                                          : "top-2 bottom-2"
                                      } ${
                                        shouldDimConflictMeeting ||
                                        shouldDimConflictByDuration
                                          ? "opacity-25"
                                          : ""
                                      }`}
                                      style={{
                              ...
                              {conflictResolution &&
                                ...
                                (() => {
                                  ...
                                  // Apply spotlight opacity for conflict meetings too
                                  const isMyDraggedMeeting =
                                    meeting.organizer === currentUser ||
                                    meeting.aiCreated;
                                  const shouldDimDraggedMeeting =
                                    spotlightMyEvents && !isMyDraggedMeeting;

                                  // Apply duration filter opacity for conflict meetings
                                  const shouldDimDraggedByDuration =
                                    activeFilters?.duration !== "any" &&
                                    activeFilters?.duration &&
                                    meeting.duration * 60 <
                                      parseInt(activeFilters.duration);
                                  ...
                                  return (
                                    <Popover
                                      open={
                                        conflictResolution.currentConfirmation ===
                                          "auto-moved" ||
                                        conflictResolution.currentConfirmation ===
                                          "dragged"
                                      }
                                      ...
                                      <PopoverTrigger asChild>
                                        <div
                                          className={`absolute ${meetingColor} rounded px-2 ${
                                            ...
                                            shouldDimDraggedMeeting ||
                                            shouldDimDraggedByDuration
                                              ? "opacity-25"
                                              : ""
                                          }`}
                                          style={{
                                                    ...
                                                    <div className="box-border content-stretch flex items-center justify-between px-[12px] py-[8px] relative w-full border-b border-[#d6d6d6] bg-white">
                                                      ...
                                                      <div className="flex items-center gap-[8px]">
                                                        ...
                                                        >
                                                          <div
                                                            aria-hidden="true"
                                                            className="absolute border border-[#d6d6d6] border-solid pointer-events-none rounded-[4px] inset-0"
                                                          />
                                                        ...
                                                        >
                                                          <div
                                                            aria-hidden="true"
                                                            className="absolute border border-[#d6d6d6] border-solid inset-0 pointer-events-none rounded-[4px]"
                                                          />
                                                          <div className="content-stretch flex gap-[8px] items-center justify-center relative">
                                                            <p className="font-['Brown_LL',_sans-serif] leading-[20px] not-italic relative shrink-0 text-[#1c1c1c] text-[12px] text-nowrap whitespace-pre font-medium">
                                                              Next
                              ...
                              {offlineRoomResolution &&
                                offlineRoomResolution.affectedMeetings.length >
                                  0 &&
                                (() => {
                                  ...
                                  const availableRooms = updatedRooms.filter(
                                    (r) => {
                                      if (
                                        r.status === "offline" ||
                                        r.id === currentMeetingData.roomId
                                      )
                                        return false;

                                      // Check for conflicts
                                      const hasConflict = r.meetings.some(
                                        (m) => {
                                          const meetingEnd =
                                            meeting.startTime +
                                            meeting.duration;
                                          const existingEnd =
                                            m.startTime + m.duration;
                                          return (
                                            (meeting.startTime >= m.startTime &&
                                              meeting.startTime <
                                                existingEnd) ||
                                            (meetingEnd > m.startTime &&
                                              meetingEnd <= existingEnd) ||
                                            (meeting.startTime <= m.startTime &&
                                              meetingEnd >= existingEnd)
                                          );
                                        }
                                  ...
                                  const currentRoomOptionIndex =
                                    selectedRoomIndex >= 0
                                      ? selectedRoomIndex
                                      : 0;
                                  const currentRoomOption =
                                    availableRooms[currentRoomOptionIndex];
                                  ...
                                  return (
                                    <Popover open={true} modal={false}>
                                      <PopoverTrigger asChild>
                                        <div
                                          className={`absolute bg-red-100 border-2 border-red-500 rounded px-2 text-red-900 flex items-center transition-all duration-200 ${
                                            compactView
                                              ? "top-1 bottom-1"
                                              : "top-2 bottom-2"
                                          }`}
                                        ...
                                        <div className="space-y-4">
                                          <div>
                                            ...
                                            <p className="text-sm text-gray-600 mt-1">
                                              {currentMeetingData.roomName} is
                                              offline. Please select a new room
                                              for this meeting.
                                            </p>
                                          </div>
                                                      ...
                                                      {(() => {
                                                        const offlineRoom =
                                                          updatedRooms.find(
                                                            (r) =>
                                                              r.id ===
                                                              currentMeetingData.roomId
                                                          );
                                                        if (!offlineRoom)
                                                          return null;
                                                        const amenitiesCount =
                                                          offlineRoom.features
                                                            ?.length || 0;
                                                        return (
                                                          <div className="flex flex-col gap-[4px]">
                                                              ...
                                                              {amenitiesCount >
                                                                ...
                                                                <div className="flex items-center gap-[4px]">
                                                                  ...
                                                                  <Tooltip>
                                                                    ...
                                                                    <TooltipContent>
                                                                      <ul className="text-xs space-y-1">
                                                                        {currentRoomOption.features.map(
                                                                          (
                                                                            feature,
                                                                            idx
                                                                          ) => (
                              ...
                              {offlineRoomResolution &&
                                ...
                                (() => {
                                  ...
                                  return (
                                    <div
                                      className={`absolute border-2 border-[#2774C1] border-dashed rounded px-2 flex items-center transition-all duration-200 ${
                                        compactView
                                          ? "top-1 bottom-1"
                                          : "top-2 bottom-2"
                                      }`}
                                    ...
                                    >
                                      <div className="min-w-0 flex-1">
                                        <div
                                          ...
                                          className="flex items-center gap-1.5 text-[#2774C1]"
                                        >
                                          <FontAwesomeIcon
                                            icon={faArrowRight}
                                            className="w-3 h-3 shrink-0"
                                        ...
                                        {!compactView && (
                                          <div
                                            style={{ fontSize: "11px" }}
                                            className="opacity-90 truncate text-[#2774C1] mt-0.5"
                                          >
                                            {meeting.organizer}
                                          </div>
                                        )}
                              ...
                              {meetingPreview &&
                                ...
                                (() => {
                                  ...
                                  return (
                                    <div
                                      className={`absolute bg-[#2774C1] rounded px-2 text-white flex items-center cursor-move ${
                                        compactView
                                          ? "top-1 bottom-1"
                                          : "top-2 bottom-2"
                                      }`}
                              ...
                              {aiMeetingPreview &&
                                ...
                                (() => {
                                  const startCol =
                                    aiMeetingPreview.startTime * 2; // Each hour = 2 columns (30-min intervals)
                                  const widthCols =
                                    aiMeetingPreview.duration * 2; // Duration in hours * 2 columns per hour
                                  const leftPercent = (startCol / 48) * 100;
                                  const widthPercent = (widthCols / 48) * 100;
              ...
              <div className="w-80 bg-white border-l border-[#D6D6D6] flex flex-col">
                <div className="border-b border-[#D6D6D6] px-[16px] py-[12px]">
                  <div className="flex items-center">
                    <FontAwesomeIcon
                      icon={faExclamationTriangle}
                      className="w-5 h-5 text-[#E81C1C] mr-2"
                    />
                    <h3 className="font-semibold text-[rgba(0,0,0,0.95)]">
                      Conference Room B Issues
                    </h3>
                  </div>
                </div>
                ...
                <div className="flex-1 p-[16px] space-y-4">
                  <div className="bg-[#fef2f2] border border-[#fecaca] rounded-lg p-4">
                    <div className="flex items-center mb-2">
                      <FontAwesomeIcon
                        icon={faTv}
                        className="w-4 h-4 text-[#dc2626] mr-2"
                      />
                    ...
                    <p className="text-xs text-[#b91c1c]">
                      Projector malfunction reported in Conference Room B. Room
                      has been taken offline for repairs.
                    </p>
                  </div>

                  <div>
                    <h4 className="font-medium text-[rgba(0,0,0,0.95)] mb-3">
                  ...
                  <div className="bg-[#f2f8ec] border border-[#d4eac3] rounded-lg p-3">
                    <div className="flex items-center">
                      <FontAwesomeIcon
                        icon={faWandSparkles}
                        className="w-4 h-4 text-[#5a8c28] mr-2"
                      />
                      <span className="text-sm font-medium text-[#3a5c18]">
                        All Clear
                      </span>
                    </div>
                    <p className="text-xs text-[#4a7420] mt-1">
                      No further action required. All affected meetings have
                      been automatically relocated to available rooms.
                    </p>
                  </div>
                </div>
              </div>
