...
interface MeetingSpacesPageProps {
  ...
  sidebarState?: 'none' | 'ai-assistant' | 'meeting-details' | 'create-meeting' | 'filters' | 'room-details';
  onSidebarStateChange?: (type: 'none' | 'ai-assistant' | 'meeting-details' | 'create-meeting' | 'filters' | 'room-details', wasAiAssistant?: boolean) => void;
...
export function MeetingSpacesPage({ 
  ...
  return (
    <PageLayout
      ...
      onWorkplaceExpandedChange={onWorkplaceExpandedChange}
                    ...
                    <div key={room.id} className={`relative border-b border-[#D6D6D6] ${rowBgColor} ${compactView ? 'h-10' : 'h-16'}`}>
                      ...
                      {room.meetings
                        ...
                        const meetingElement = (
                          <div
                            key={meeting.id}
                            className={`meeting-block absolute rounded px-2 flex items-center transition-all duration-200 ${compactView ? 'top-1 bottom-1' : 'top-2 bottom-2'} ${
                              isSyncing 
                          ...
                          >
                            <div 
                              className={`flex-shrink-0 transition-all duration-200 ${
                                isSyncing || meeting.pendingApproval || isMeetingPast(meeting)
                                  ? 'opacity-0 w-0 mr-0' 
                            ...
                            <div className={`min-w-0 flex-1 transition-all duration-200 ${
                              hoveredMeeting === meeting.id ? 'ml-0' : 'ml-0'
                            }`}>
                        ...
                        return (
                          <Popover open={true} modal={false}>
                            <PopoverTrigger asChild>
                              <div
                                className={`absolute ${meetingColor} rounded px-2 ${pendingMove.meeting.pendingApproval ? 'text-[#2774C1] border-2 border-[#2774C1]' : isMeetingFuture(pendingMove.meeting) ? 'text-[#2774C1]' : 'text-white'} flex items-center transition-all duration-200 ${compactView ? 'top-1 bottom-1' : 'top-2 bottom-2'} ${
                                  shouldDimPendingMeeting || shouldDimPendingByDuration ? 'opacity-25' : ''
                                }`}
                                style={{
                                  left: `calc(${leftPercent}% + 2px)`,
                                  width: `calc(${widthPercent}% - 4px)`,
                      ...
                      {conflictResolution && conflictResolution.draggedMeeting.targetRoomId === room.id && (() => {
                        ...
                        return (
                          <Popover open={conflictResolution.currentConfirmation === 'auto-moved' || conflictResolution.currentConfirmation === 'dragged'} modal={false}>
                            <PopoverTrigger asChild>
                              <div
                                className={`absolute ${meetingColor} rounded px-2 ${meeting.pendingApproval ? 'text-[#2774C1] border-2 border-[#2774C1]' : isMeetingFuture(meeting) ? 'text-[#2774C1]' : 'text-white'} flex items-center transition-all duration-200 ${compactView ? 'top-1 bottom-1' : 'top-2 bottom-2'} ${
                                  shouldDimDraggedMeeting || shouldDimDraggedByDuration ? 'opacity-25' : '' 
                                }`}
                                style={{
                                  left: `calc(${leftPercent}% + 2px)`,
                                  width: `calc(${widthPercent}% - 4px)`,
